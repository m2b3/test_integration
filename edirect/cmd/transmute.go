...
func main() {
	...
	*/
	...
	if args[0] == "-chunk" {
		...
		rdr := eutils.CreateTextStreamer(in)
	...
	if args[0] == "-stream" {
		...
		psrq := eutils.CreateTextProducer(pat, "", "", 0, 0, rdr)
	...
	if args[0] == "-shuffle" {
		...
		psrq := eutils.CreateTextProducer(pat, "", "", 0, 0, rdr)
		gbkq := eutils.CreateGPConverters(psrq, 0, "ARC")
		unsq := eutils.CreateTextUnshuffler(gbkq, false)
...
`
	...
	// READ GENPEPT FLATFILE AND TRANSLATE TO PEPTINFO XML

	if len(args) > 0 && args[0] == "-g2i" {
		...
		for len(args) > 0 {
			...
			switch args[0] {
			case "-base", "-start":
				startUID = eutils.GetNumericArg(args, "First UID to assign", 0, 0, math.MaxInt32)
				args = args[2:]
			case "-div", "-division":
				division = eutils.GetStringArg(args, "Division abbreviation")
				args = args[2:]
			default:
				eutils.DisplayError("Unrecognized option after -g2i command")
				os.Exit(1)
			}
		}

		// GenPept flatfiles start with LOCUS line
		pattern := "LOCUS       "

		lbsq := eutils.CreateTextStreamer(in)
		psrq := eutils.CreateTextProducer(pattern, "", "", 0, 0, lbsq)
		gbkq := eutils.CreateGPConverters(psrq, startUID, division)
		unsq := eutils.CreateTextUnshuffler(gbkq, false)

		if lbsq == nil || psrq == nil || gbkq == nil || unsq == nil {
			eutils.DisplayError("GenPept converters not built")
			os.Exit(1)
		}
