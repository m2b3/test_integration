...
func main() {
	...
	gzipHead := eutils.GzipString(pmaSetHead, false)
	gzipTail := eutils.GzipString(pmaSetTail, false)
	...
	pubmedFetch := func(c *gin.Context, uids string, tbo string) {
		...
		uidq := eutils.UIDStreamer(eutils.ChanToReader(eutils.StringToChan(uids)))
		strq := eutils.ReadArchiveRecords("pubmed", turbo, uidq)

		if uidq == nil || strq == nil {
	...
	pubmedStream := func(c *gin.Context, uids string) {
		...
		// concurrent fetching by multiple goroutines
		uidq := eutils.UIDStreamer(eutils.ChanToReader(eutils.StringToChan(uids)))
		datq := eutils.StreamArchiveRecords("pubmed", uidq)

		if uidq == nil || datq == nil {
