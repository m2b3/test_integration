...
func main() {
	...
	*/
	...
	if len(args) == 4 && (strings.HasPrefix(args[2], "-sort") || strings.HasPrefix(args[2], "-stable-")) {

		sortInReverse := false
		if strings.HasSuffix(args[2], "-rev") {
			sortInReverse = true
		}

		sortStable := false
		if strings.HasPrefix(args[2], "-sort-stable") || strings.HasPrefix(args[2], "-stable-sort") {
			sortStable = true
		}
		...
		var keys []string
		if sortStable {
			keys = slices.SortedStableFunc(maps.Keys(order), sortKeys)
		} else {
	...
	// SPLIT FILE BY RECORD SIZE

	// split XML record into subfiles by size
	if len(args) == 8 && (args[2] == "-split-by-len" || args[2] == "-allot") && args[4] == "-prefix" && args[6] == "-suffix" {
		...
		// get maximum allowable cumulative byte size
		chunk, err := strconv.Atoi(args[3])
		if err != nil {
	...
	// SPLIT FILE BY RECORD IDENTIFIER

	// split XML record into subfiles by record identifier
	if len(args) == 8 && (args[2] == "-split-by-id") && args[4] == "-prefix" && args[6] == "-suffix" {

		// e.g., -set PMCInfoSet -pattern PMCInfo -split-by-id PMCInfo/UID -prefix PMC -suffix xml
		...
		eutils.PartitionXML(topPattern, star, false, rdr,
			func(str string) {
				...
				// remove file name prefix if it is at beginning of identifier
				id = strings.TrimPrefix(id, prefix)

				// create next file
				fpath := fmt.Sprintf("%s%s.%s", prefix, id, suffix)
				fl, err = os.Create(fpath)
				if err != nil {
					eutils.DisplayError("Unable to create path '%s'", err.Error())
					return
