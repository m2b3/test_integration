...
// CreateGPConverters makes concurrent calls to GenPeptToInfo
func CreateGPConverters(inp <-chan TextRecord, startUID int, division string) <-chan TextRecord {
	...
	if out == nil {
		DisplayError("Unable to create gbp converter channel")
		os.Exit(1)
	}

	gpConvert := func(wg *sync.WaitGroup, inp <-chan TextRecord, out chan<- TextRecord) {

		// report when this instance has no more records to process
		defer wg.Done()

		var rec strings.Builder

		genPeptToInfo := func(txt string, idx int) string {
			...
			*/
			...
			writeOpenElement := func(spaces, tag, value string) {
			...
			writeAttribute := func(tag, value string) {
			...
			writeCloseElement := func(tag, value string) {
			...
			writeOneElement := func(spaces, tag, value string) {
				...
				writeOpenElement(spaces, tag, value)
				writeCloseElement(tag, value)
			}

			rec.WriteString("<PeptInfo>\n")

			number := strconv.Itoa(startUID + idx)
			writeOneElement("  ", "UID", number)

			div := ""
			if strings.HasPrefix(txt[:], "LOCUS") {
				pos := strings.Index(txt[:], "\n")
				if pos >= 0 {
					cols := strings.Fields(txt[:pos])
					ln := len(cols)
					if ln == 8 {
						div = cols[6]
					} else if ln == 7 {
			...
			defline := ""
			pos := strings.Index(txt[:], "DEFINITION  ")
			if pos >= 0 {
				pos += 12
				defline = txt[pos:]
				txt = txt[pos:]
			...
			accession := ""
			pos = strings.Index(txt[:], "ACCESSION   ")
			if pos >= 0 {
				if defline != "" {
					defline = defline[:pos]
					defline = strings.Replace(defline, "\n", " ", -1)
					defline = CompressRunsOfSpaces(defline)
					defline = strings.TrimSpace(defline)
				}
				pos += 12
				accession = txt[pos:]
				txt = txt[pos:]
				pos = strings.Index(accession[:], "\n")
				if pos > 0 {
					accession = accession[:pos]
					// trim to first space, if present, to ignore secondary accessions
					pos = strings.Index(accession[:], " ")
					if pos > 0 {
						accession = accession[:pos]
					}
					writeOneElement("  ", "Accession", accession)
				}
			}

			organism := ""
			pos = strings.Index(txt[:], "  ORGANISM  ")
			if pos >= 0 {
				pos += 12
				organism = txt[pos:]
				txt = txt[pos:]
				pos = strings.Index(organism[:], "\n")
				if pos > 0 {
					organism = organism[:pos]
				}
			}

			taxid := ""
			pos = strings.Index(txt[:], "     source          ")
			if pos >= 0 {
				pos += 21
				txt = txt[pos:]
				pos = strings.Index(txt[:], "                     /db_xref=\"taxon:")
				if pos > 0 {
					pos += 37
					taxid = txt[pos:]
					txt = txt[pos:]
					pos = strings.Index(taxid[:], "\"")
					if pos > 0 {
						taxid = taxid[:pos]
						taxid = strings.Replace(taxid, "\n", " ", -1)
						taxid = CompressRunsOfSpaces(taxid)
			...
			if organism != "" {
				writeOpenElement("  ", "Organism", organism)
				if division != "" {
					writeAttribute("div", division)
				}
				if taxid != "" {
					writeAttribute("taxid", taxid)
				}
				writeCloseElement("Organism", organism)
			}

			product := ""
			pos = strings.Index(txt[:], "     Protein         ")
			if pos >= 0 {
				pos += 21
				txt = txt[pos:]
				pos = strings.Index(txt[:], "                     /product=\"")
				if pos > 0 {
					pos += 31
					product = txt[pos:]
					txt = txt[pos:]
					pos = strings.Index(product[:], "\"")
					if pos > 0 {
						product = product[:pos]
						product = strings.Replace(product, "\n", " ", -1)
						product = CompressRunsOfSpaces(product)
			...
			if defline != "" {
				writeOneElement("  ", "Product", product)
			}

			sequence := ""
			pos = strings.Index(txt[:], "ORIGIN")
			if pos >= 0 {
				pos += 6
				sequence = txt[pos:]
				...
				writeOneElement("  ", "Sequence", sequence)
			}
		...
		for curr := range inp {
			...
			txt := genPeptToInfo(curr.Text, curr.Index)
	...
	// launch multiple gp converter goroutines
	for range numProcs {
		wg.Add(1)
		go gpConvert(&wg, inp, out)
	}
...
func GenBankConverter(inp io.Reader) <-chan string {
	...
	go func(inp io.Reader, out chan<- string) {
