...
var legalFeats map[string]string
var legalQuals map[string]string
var legalINSDs map[string]string
...
func ProcessINSD(args []string, isPipe, addDash, doIndex, makeXML bool) []string {
	...
	checkAgainstVocabulary := func(str, objtype string, legal map[string]string) {
		...
		ky := strings.ToUpper(str)
		val, ok := legal[ky]
		if ok {
			if str == val {
				return
			}
			DisplayError("Incorrect capitalization of '%s' %s, change to '%s'", str, objtype, val)
			os.Exit(1)
		}
	...
	if strings.HasPrefix(args[0], "INSD") {
		...
		for {
			...
			checkAgainstVocabulary(str, "element", legalINSDs)
			acc = append(acc, str)
			args = args[1:]
		}
		...
		checkAgainstVocabulary(args[0], "element", legalINSDs)
	...
	processOneFeature := func(ftargs []string) {
		...
		plus := strings.SplitSeq(feature, "+")
		for pls := range plus {
			comma := strings.SplitSeq(pls, ",")
			for cma := range comma {

				checkAgainstVocabulary(cma, "feature", legalFeats)
		...
		for _, str := range ftargs {
			...
			if strings.HasPrefix(str, "INSD") {
				...
				checkAgainstVocabulary(str, "qualifier", legalQuals)
				if doIndex {
...
func init() {
	...
	legalFeats = make(map[string]string)
	legalQuals = make(map[string]string)
	legalINSDs = make(map[string]string)

	for _, str := range feats {
		ky := strings.ToUpper(str)
		legalFeats[ky] = str
	}

	for _, str := range quals {
		ky := strings.ToUpper(str)
		legalQuals[ky] = str
	}
	for _, str := range insds {
		ky := strings.ToUpper(str)
		legalINSDs[ky] = str
	}
}
