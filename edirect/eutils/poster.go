...
// now cannot be calculated without unpacking (or scanning) the variable length
// data.) The number of PMIDs per term is the document frequency (DF). All that
// remains for calculating TF-IDF term weights, for supporting ranked retrieval,
...
func CreatePromoters(prom, db, fields string, isLink bool, files []string) <-chan string {
	...
	xmlPromoter := func(wg *sync.WaitGroup, fileName string, out chan<- string) {
			...
			for _, str := range recs {
				...
				if tag == "" {
					if isLink {
						...
						tag = IdentifierKey(term, field)
						// underscore is only for file name, revert to space for proper tag sorting
		...
		getIndexedFieldNames := func(recs, flds []string) []string {

			fieldsSeen := make(map[string]bool)

			for _, str := range recs {
				StreamValues(str[:], "InvDocument",
					func(tag, attr, content string) {
						fieldsSeen[tag] = true
					})
			}

			fieldsSeen["InvKey"] = false
			fieldsSeen["InvIDs"] = false

			requestedFields := make(map[string]bool)

			for _, fld := range flds {
				requestedFields[fld] = true
			}
			...
			for field := range fieldsSeen {
				...
				if pos > 0 {
					// remove characters starting from hyphen, e.g., "ACCN-XP" -> "ACCN"
					fld = fld[:pos]
				}
				if requestedFields[fld] {
					allowedFields[field] = true
				}
			}

			keys := slices.Sorted(maps.Keys(allowedFields))
		...
		PartitionXML("InvDocument", "", false, rdr,
			func(str string) {
				...
				id, fld, tag, _ := parseInvDocument(str[:])
				if id == "" {
					return
				}
				...
				if isLink {
					...
					// use first few characters of identifier
					currTag = IdentifierKey(id, fld)
					// underscore is only for file name, revert to space for proper tag sorting
					currTag = strings.Replace(currTag, "_", " ", -1)
					if currTag != tag {
						DisplayError("InvDocument tag '%s' != currTag '%s'", tag, currTag)
					}
				}
				...
				if prevTag != currTag {
					...
					if prevTag != "" {
						idxflds := getIndexedFieldNames(arry, flds)
						for _, key := range idxflds {
							processOneField(key, arry)
						}
						out <- prevTag
					}
