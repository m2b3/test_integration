...
func ResolveArchivePaths(db string) LclArchPathMap {
	...
	// top-level groups (ARCHIVE, WORKING, POSTING, INVERTS, and SOURCES) allow separate volumes
	// or paths for related subfolders, read from configuration file or environment variables
		...
		*/
		...
		if text != "" {
			...
			if pat != nil {
				...
				sections := []string{"ARCHIVE", "WORKING", "POSTING", "INVERTS", "SOURCES"}

				visitFunc := func(sect *XMLNode) {

					// set top-level group paths from configuration section
					for _, dr := range sections {
						VisitElements(sect, dr, "", func(str string) {
							topPaths[dr] = strings.TrimSuffix(str, "/")
						})
					}
				}

				// limit to database section
				VisitNodes(pat, db, visitFunc)

				if topPaths["ARCHIVE"] == "" {
					// try default section
					VisitNodes(pat, "default", visitFunc)
				}
			}
		}
	}
	...
	if topPaths["ARCHIVE"] == "" {
		...
		topPaths["INVERTS"] = getEnvPath("EDIRECT_LOCAL_INVERTS")
	...
	// Indexed and inverted files can also be in a separate location to avoid disk overflow on large datasets
	if topPaths["INVERTS"] == "" {
		topPaths["INVERTS"] = topPaths["WORKING"]
	}
	...
	for _, dr := range []string{"Extras", "Merged", "Scratch"} {
		pth := topPaths["WORKING"] + "/" + db + "/" + dr
		res[dr] = pth
	}

	for _, dr := range []string{"Collect", "Current", "Indexed", "Inverted", "Temporary"} {
	...
	res["Invert"] = topPaths["INVERTS"] + "/" + db + "/Invert"
...
func printTermCounts(base, term, field string) int {
	...
	pdlen := len(PostingDir(term, field))
...
func parseField(db, str string) (string, string) {
	...
	if strings.HasSuffix(str, "]") {
		...
		case "STEM", "TIAB", "TITL", "ABST", "TEXT", "TERM":
			// running xtract -indexer on these fields also removes stopwords
		case "PIPE":
		case "PROD":
			// !!! temporarily allow phrase searching of peptide PROD field, without
			// stopword removal - still need to check if they are removed from query
		default:
			// !!! this line prevents phrase searches
			// To make behavior automatic, will need to look at a setting in the configuration
			// file, or examine the postings area for presence of *.ofs and *.uqi files
...
func evaluateQuery(base, db, phrase string, clauses []string, noStdout, isLink bool) (int, []int32) {
	...
	eval := func(str string) ([]int32, [][]uint32, int) {
		...
		if field == "PIPE" {
			...
			// efetch -format uid | xsearch -query "[PIPE] AND coagulation [TITL]"
			var data []int32
			// read UIDs from stdin
			uidq := UIDReader(os.Stdin)
			for ext := range uidq {
...
func prepareQuery(str string) string {
	...
	// restore bracketed field name containing a single hyphen
	var arry []string

	prev := ""
	for _, curr := range terms {
		...
		if strings.HasSuffix(curr, "]") {
			if prev != "" {
				arry = append(arry, prev+"-"+curr)
				prev = ""
				continue
			}
		}
		if prev != "" {
			arry = append(arry, prev)
			prev = ""
		}
		if curr != "" {
	...
	if prev != "" {
		// handle unbalanced opening bracket in last term
		arry = append(arry, prev)
	}

	// rejoin into processed sentence
	tmp := strings.Join(arry, " ")
