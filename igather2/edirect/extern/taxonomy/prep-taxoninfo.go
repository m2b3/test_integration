...
func createTAXONINFO(verboseAncestors, verboseChildren bool) int {
	...
	readRankedLineage := func(cols []string) {
		...
		taxID, taxName, species, genus, family, order, class, phylum, kingdom, domain :=
			cols[0], cols[2], cols[4], cols[6], cols[8], cols[10], cols[12], cols[14], cols[16], cols[18]
	...
	// builder writes to stdout in chunks for efficiency
	var bldr strings.Builder
	count := 0
	okay := false
	...
	printWithAttribs := func(name, tag1, val1, tag2, val2, value string) {
		...
		if value != "" {
			...
			if fst {
				...
				bldr.WriteString(head)
				fst = false
			}

			// indent appropriate number of spaces
			bldr.WriteString(spaces)
			...
			if tag1 != "" && val1 != "" {
				bldr.WriteString(" ")
				bldr.WriteString(tag1)
				bldr.WriteString("=\"")
				bldr.WriteString(val1)
				bldr.WriteString("\"")
			}
			if tag2 != "" && val2 != "" {
				bldr.WriteString(" ")
				bldr.WriteString(tag2)
				bldr.WriteString("=\"")
				bldr.WriteString(val2)
				bldr.WriteString("\"")
			}
			bldr.WriteString(">")

			bldr.WriteString(value)
	...
	for _, key := range keys {
		...
		if rnk.Domain != "" {
			printTree([]string{
				rnk.Domain, rnk.Kingdom, rnk.Phylum, rnk.Class, rnk.Order, rnk.Family, rnk.Genus, rnk.Species,
			})
		}
		...
		if tn.TaxID != "1" {
			...
			pn := taxonInfoMap[pID]
			if verboseAncestors {
				printWithAttribs("ParentID", "name", pn.Scientific, "", "", pID)
			} else {
				printOne("ParentID", pID)
			}
		}
